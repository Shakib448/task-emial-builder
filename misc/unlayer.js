  ...
  function (canCreateDiscussions, isSlidingUp) {
    ...
    (function (scope) {
      ...
      function parse(input, stateOverride, base) {
        ...
        e: for (
          ;
          (input[cursor - 1] != EOF || 0 == cursor) && !this._isInvalid;
        ...
        Object.defineProperty(doc, "currentScript", {
          get: function () {
            try {
              ...
              var i;
              var _src_temp = (/.*at [^\(]*\((.*):.+:.+\)$/gi.exec(
                error.stack
              ) || [false])[1];
              for (i in scripts) {
                if (
                  scripts[i].src == _src_temp ||
                  "interactive" == scripts[i].readyState
                ) {
                  return scripts[i];
                }
              }
        ...
        (match =
          ...
          typeof (test = function () {
            ...
            function parse(n) {
              ...
              var index =
                ((dateDelim = "-"),
                -1 === (value = line).indexOf(dateDelim)
                  ? value.length
                  : value.indexOf(dateDelim));
            ...
            function i(val) {
              ...
              if (!rNumpx.test(val)) {
                throw new Error(
                  "Invalid argument not valid semver ('" + val + "' received)"
                );
              }
            }
            /**
            ...
            function b(b, a) {
              ...
              for (
                ;
                prop < Math.max(output.length - 1, date.length - 1);
                prop++
              ) {
                /** @type {number} */
              ...
              if (o && d) {
                ...
                for (; prop < Math.max(a.length, b.length); prop++) {
                  if (
                    void 0 === a[prop] ||
                    ("string" == typeof b[prop] && "number" == typeof a[prop])
                  ) {
                    return -1;
                  }
                  if (
                    void 0 === b[prop] ||
                    ("string" == typeof a[prop] && "number" == typeof b[prop])
                  ) {
                    return 1;
                  }
                  if (a[prop] > b[prop]) {
  ...
  function (canCreateDiscussions, res, self) {
    ...
    var StringIO = (function () {
      ...
      return (
        ...
        (fObPool = [
          ...
          },
          {
            ...
            value: function ($) {
              ...
              i = contactCapacity;
              if (
                ("dev" === (setwidth = width) ? 1 : self(4)(setwidth, i)) <= 0
              ) {
                /** @type {boolean} */
                dayMode = false;
              }
            ...
            value: function (options) {
              ...
              if (
                (options.id
                  ...
                  : options.className &&
                    (d = document.getElementsByClassName(options.className)[0]),
                !options.id && !options.className)
              ) {
        ...
        ]) && defineProperty(self.prototype, fObPool),
        i && defineProperty(self, i),
        callback
      );
    })();
